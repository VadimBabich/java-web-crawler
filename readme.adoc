...
xref:web-crawler-test/src/main/docker/app.yml[]
[source,yml]
----
include::web-crawler-test/src/main/docker/app.yml[]
----
`Grafana` dashboards for `Influxdb` datasource are also available in the test-application.

image::doc/img/pic_processing_metrics.png[caption="http://localhost:3000 " title="Pages processing - rate, duration and size"]

image::doc/img/pic_services_metrics.png[caption="http://localhost:3000 " title="Services (interceptors) - rate and duration"]
You can use a properties file to configure the database connection and registry settings. The file path can be passed to the class constructor, or a default value can be used for the local database instance.
...
The property file influx.properties looks like this:
[source,properties]
----
        influx.db=crawler
